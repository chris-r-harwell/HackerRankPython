...
"""
# A list of vowels for the purposes of minion game
# intentionally skipping Y.
vowels = ('A', 'E', 'I', 'O', 'U')
debug = False
...
def minion_game(string):
    ...
    stuart = 0
    length = len(string)
    dprint('length = ' + str(length))
    for start in range(0, length):
        for end in range(start + 1, length + 1):
            dprint('start: {!s}'.format(start))
            dprint('end: {!s}'.format(end))
            substring = s[start:end]
            dprint('substring: {}'.format(substring))
            if substring[0] in vowels:
                kevin += 1
            else:
                stuart += 1
            dprint('stuart: {!s} kevin: {!s}'.format(stuart, kevin))
            dprint('')
